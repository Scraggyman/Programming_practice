# Программа, которая читает данные из файла в формате .csv и выводит таблицу в HTML

import csv
import sys

def main():
    maxwidth = 100 # число символов в ячейке
    print_start()
    count = 0
    while True:
        try:
...
def print_line(line, color, maxwidth):
    print("<tr bdcolor='{0}'>".format(color))
...
def extract_fields():
    ...
    for c in line:
        if c in "\"'":
            ...
            continue
        if quote is None and c == ",": # конец строки
            fields.append(field)
            field = ""
        else:
